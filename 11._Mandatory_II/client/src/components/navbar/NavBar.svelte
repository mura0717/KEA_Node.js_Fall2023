<script>
  ...
  let userStatus = 0;
  $: userStatus = $user ? $user.isAdmin : 0;

  export let Link;

  async function handleToAdminPage() {
    if(userStatus == 1){
      navigate("/auth/admin");
    } else {
   ...
   async function handleToSecretPage() {
    if(userStatus == 1){
      navigate("/auth/admin/secret");
    } else {
      toast.error("Unauthorized. Admin only.")
    }
  } 
...
<nav
  class="relative flex w-full flex-wrap items-center justify-between bg-[#FBFBFB] py-2 text-neutral-500 shadow-lg hover:text-neutral-700 focus:text-neutral-700 dark:bg-neutral-600 lg:py-1"
  data-te-navbar-ref
>
  <div
